// PG-NOTES:
// > This file shouldn't be 910 lines long
//  > Pull out functions into separate files
...
const {
  ...
  EmbedBuilder,
  AttachmentBuilder,
} = require("discord.js"); //discord connection
...
const carl =
  "https://cdn.discordapp.com/avatars/971503249138008134/c2abcb03ecba9ec9169b51667a67e507.png";

// PG-NOTES: - Initiate client and db object elsewhere
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (commandFile) {
    ...
    // PG-NOTES: Don't pass pgClient around, just import it in the command file
...
const pglogo =
  "https://cdn.discordapp.com/attachments/933487341824270356/1031769217235701781/PGlogo_whitebg.png";
...
// PG-NOTES: - Utility functions in a separate directory/file
...
function getNopemoji() {
  ...
  // PG-NOTES: Just do
  // return unshuffled[Math.floor(Math.random() * unshuffled.length)];
  // let shuffled = unshuffled
  // .map(value => ({ value, sort: Math.random() }))
  // .sort((a, b) => a.sort - b.sort)
  // .map(({ value }) => value);

  var randomIndex = getRndInteger(0, unshuffled.length);

  return unshuffled[randomIndex];
}
...
async function createGif(avatarUrls, winnerUserIDs) {
  ...
  for (let cycle = 1; cycle <= 8; cycle++) {
    ...
    const avatarPromises = avatarUrls.map(async (avatarUrl, index) => {
      ...
      if (!response.ok) {
        console.error(
          `Failed to fetch avatar for ${avatarUrl}. Status: ${response.status}`,
        );
    ...
    if (cycle === 8) {
      ...
      for (let i = 0; i < winnerUserIDs.length; i++) {
        ...
        try {
          ...
          console.error(
            `Error fetching avatar for ${winnerUserIDs[i]}: ${error.message}`,
          );
          console.log("Assigning default avatar...");
          winner = noAvatar;
        }
        ...
        if (!winnerResponse.ok) {
          console.error(
            `Failed to fetch winner's avatar. Status: ${winnerResponse.status}`,
          );
  ...
  try {
    ...
    const gifBuffer = await createGif(avatarUrls, userIds).then((gifBuffer) => {
      ...
      const raffleEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `*The following entries have been processed and ${descStr} been selected...*`,
        )
...
client.on("messageCreate", async (message) => {
  ...
  try {
    (server = message.guild.id), (channel = message.channel.id);
  } catch (err) {
  ...
  if (message.author.id == bitcorn || message.author.id == agentX3) {
    if (message.content.startsWith("!sql")) {
      ...
      try {
        ...
        // PG-NOTES: Hardcoded OS-specific path
        // queryFile = path.join("createdfiles", "query.txt")
        fs.writeFileSync(
          ".\\createdfiles\\query.txt",
        ...
        await message.reply({
          content: `Data saved in attached query.txt`,
          files: [attachment],
        });
      } catch (error) {
    ...
    if (message.member.roles.cache.has("970758538681012315")) {
      if (message.type !== "REPLY") {
        await message.channel.send(
          "Reply to the list of users you wish to process.",
        );
        return;
      }
      ...
      message
        ...
        .then(async (repliedTo) => {
          ...
          var playerselected = randomSelect(
            repliedTo.content.split(" "),
            winners,
          );
          if (playerselected.length == 0) {
            throw "zero-length string error";
          ...
          // Create GIFs for winners
          const gifBuffer = await createGif(avatarUrls, userIds).then(
            (gifBuffer) => {
              console.log("Editing initial message...");
              const titlestr =
                winners === 1 ? "OUR WINNER!!!" : "OUR WINNERS!!!";
              sentMessage.edit(
              ...
              if (winners === 1) {
                ...
                if (additionalWinners.length > 0) {
                  ...
                  msgtext += `\n...and our remaining winners:\n`;
                  for (let i = 0; i < additionalWinners.length; i++) {
                    msgtext += `${winnerEmoji} WINNER ${
                      i + 4
                    }  ${arrowright}   <@${additionalWinners[i]}>\n`;
                    if (winnerEmoji == partydoggo) {
                      winnerEmoji = danceboy;
                    } else {
                      winnerEmoji = partydoggo;
