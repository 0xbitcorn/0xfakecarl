...
const fs = require("fs");
const { gameStats } = require("../../gameStats.mjs");
  ...
  async execute(interaction, pgClient) {
    try {
      ...
      const latestPuzzlesQuery = `
                  ...
                  LIMIT 15`;
      const latestPuzzlesResult = await pgClient.query(latestPuzzlesQuery, [
        puzzleKind,
      ]);
      ...
      const filter = (i) =>
        i.customId === "selectPuzzle" && i.user.id === interaction.user.id;
